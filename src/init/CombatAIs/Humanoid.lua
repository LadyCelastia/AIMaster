...
HumanoidAI.new = function(Character: Model)
	...
	self.Active = false
	self.Frames = 20 -- how many frames per update
	self.CurrentFrame = 0
	self.Aggression = {}
				...
				for timestamp, aggression in pairs(self.Aggression[object.Value]) do
					...
					if tick() - timestamp < 5 then
						...
						if totalAggression > self.CurrentAggression then
							...
							self.StateMachine.Character:FindFirstChildOfClass("Humanoid"):MoveTo(self.StateMachine.Path.Waypoints[self.StateMachine.Path.Next])
						end
					else
			...
			if self.StateMachine ~= nil and self.StateMachine.Character ~= nil and self.Active == true then
				if self.StateMachine.State == "Idle" then
					...
					for _,v in ipairs(workspace:GetChildren()) do
						if v:IsA("Model") and v:FindFirstChildOfClass("Humanoid") and v:FindFirstChild("Team") and v:FindFirstChild("HumanoidRootPart") then
							local team = v:FindFirstChild("Team")
							local hum = v:FindFirstChildOfClass("Humanoid")
							local targetRoot = v:FindFirstChild("HumanoidRootPart")
							if hum.Health > 0 and Teams.GetRelationStatus(self.StateMachine.Character.Team.Value, team.Value) == 0 and (self.StateMachine.Character.HumanoidRootPart.Position - targetRoot.Position).magnitude < dist then
								closest = v
								dist = (self.StateMachine.Character.HumanoidRootPart.Position - targetRoot.Position).magnitude < dist
							end
						end
					end
					if closest ~= nil then
				...
				elseif self.StateMachine.State == "Chasing" then
					...
					if self.StateMachine.Target == nil then
						...
						if targetRoot and hum and hum.Health > 0 then
							if (targetRoot.Position - self.StateMachine.Character.HumanoidRootPart.Position <= 4.5) then
